...
def _sample_row() -> dict:
    return {
        ...
        "Total_TL_opened_L6M": 2, "Tot_TL_closed_L6M": 0,
        "pct_tl_open_L6M": 16.7, "pct_tl_closed_L6M": 0.0,
        "pct_active_tl": 66.7, "pct_closed_tl": 33.3,
        "Total_TL_opened_L12M": 3, "Tot_TL_closed_L12M": 1,
        "pct_tl_open_L12M": 25.0, "pct_tl_closed_L12M": 8.3,
        "Tot_Missed_Pmnt": 0,
...
class TestEngineerFeatures:
    def test_delinquency_rate_no_delinquency(self):
        ...
        df = pd.DataFrame([{**_sample_row(), "num_times_delinquent": 0, "Total_TL": 10}])
        out = engineer_features(df)
        assert out["delinquency_rate"].iloc[0] == pytest.approx(0.0 / 11, abs=1e-6)

    def test_delinquency_rate_with_delinquency(self):
        from src.training.train_pipeline import engineer_features
        df = pd.DataFrame([{**_sample_row(), "num_times_delinquent": 3, "Total_TL": 10}])
        out = engineer_features(df)
    ...
    def test_dpd_severity_score(self):
        from src.training.train_pipeline import engineer_features
        df = pd.DataFrame([{**_sample_row(), "num_times_30p_dpd": 2, "num_times_60p_dpd": 1}])
        out = engineer_features(df)
    ...
    def test_engineered_columns_present(self):
        from src.training.train_pipeline import engineer_features
        expected = [
            "delinquency_rate", "recent_activity_ratio", "credit_history_span",
            "dpd_severity_score", "enquiry_acceleration",
            "has_delinquency_history", "has_payment_history",
            "has_enquiry_history", "has_credit_history",
        ]
...
class TestPrepareXY:
    ...
    def test_drop_columns_removed(self):
        from src.training.train_pipeline import engineer_features, prepare_xy
        row = {**_sample_row(), "Approved_Flag": "P1",
               "Credit_Score": 700, "PROSPECTID": 999}
    ...
    def test_no_nan_in_enquiry_cols(self):
        from src.training.train_pipeline import (
            ENQ_COUNT_COLS, engineer_features, prepare_xy,
        )
        row = {**_sample_row(), "Approved_Flag": "P1", "tot_enq": np.nan, "enq_L3m": np.nan}
        df = engineer_features(pd.DataFrame([row]))
        X, _ = prepare_xy(df)
