...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["has_delinquency_history"] = (
        df["time_since_recent_deliquency"].notna().astype(int)
    )
...
def evaluate(
    name: str, pipeline: Pipeline, X_train, y_train, X_test, y_test, sample_weight=None
) -> dict:
    fit_params = {}
    ...
    logger.info(
        "\n%s",
        classification_report(
            y_test, y_pred_test, target_names=["P1", "P2", "P3", "P4"]
        ),
    )
...
def tune_xgboost(X_train, y_train, sample_weight=None) -> Pipeline:
    logger.info(
        "Running RandomizedSearchCV for XGBoost (n_iter=%d, cv=%d)",
        TUNE_N_ITER,
        TUNE_CV_FOLDS,
    )
    ...
    search = RandomizedSearchCV(
        ...
        scoring="f1_macro",
        cv=StratifiedKFold(
            n_splits=TUNE_CV_FOLDS, shuffle=True, random_state=RANDOM_STATE
        ),
        verbose=1,
        n_jobs=-1,
        random_state=RANDOM_STATE,
...
def explain_with_shap(pipeline: Pipeline, X_test: pd.DataFrame) -> str:
    ...
    if isinstance(shap_values, list):
        shap.summary_plot(
            shap_values[2], X_df, show=False, plot_type="beeswarm", max_display=20
        )
    else:
        shap.summary_plot(
            shap_values, X_df, show=False, plot_type="beeswarm", max_display=20
        )
...
def main():
    ...
    with mlflow.start_run(run_name="XGBoost_Tuned_Weighted") as run:
        ...
        cm_path = plot_confusion_matrix(
            y_test, r_xgb["y_test_pred"], "XGBoost_Weighted"
        )
        ...
        [
            {
                k: v
                for k, v in r.items()
                if k not in ["pipeline", "y_test_pred", "report"]
            }
